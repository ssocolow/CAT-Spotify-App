...
# Initialize lists to store country codes and names
country_codes = []
country_names = []

# Read the CSV data
csv_data = '''name,alpha-2,alpha-3,country-code,iso_3166-2,region,sub-region,intermediate-region,region-code,sub-region-code,intermediate-region-code
...
for row in reader:
    country_codes.append(row['alpha-2'])
    country_names.append(row['name'])

# Convert lists to JSON strings
js_country_codes = json.dumps(country_codes)
js_country_names = json.dumps(country_names)

# Create JavaScript code
js_code = f"""
const countryCodes = {js_country_codes};
const countryNames = {js_country_names};
"""
